CLASS zcl_generate_bookings_eou DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_GENERATE_BOOKINGS_EOU IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_eou.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_eou.
*   insert the new table entries
    INSERT ztbooking_eou FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_eou INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
